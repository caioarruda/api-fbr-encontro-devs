...
const isAuthenticated = async (resolve: any, root: any, args: any, context: any, info: any) => {
    try {
        if (!context.token) throw new Error('Token não informado')
        let authenticated = await verifyAuthenticated(context.token, context)
...
const generateToken = async (resolve: any, root: any, args: any, context: any, info: any) => {
    try {
